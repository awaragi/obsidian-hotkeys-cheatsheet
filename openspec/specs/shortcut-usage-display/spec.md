...
Each hotkey entry SHALL display a usage indicator consisting of one Unicode "eighths block" character (`▁▂▃▄▅▆▇█`) plus the entry's raw press count as text, whenever `trackShortcutUsage` is `true`. The glyph level SHALL be computed from the entry's count relative to the highest single-entry count across the full dataset. An entry with a bound hotkey but zero recorded presses SHALL still render the indicator, showing the lowest glyph level and `0`, in its normal alphabetical/category position — there is no separate "unused" grouping.

#### Scenario: Entry with presses shows a proportional indicator
- **WHEN** `trackShortcutUsage` is `true` and an entry has a recorded count greater than zero
- **THEN** the entry displays a glyph scaled relative to the highest entry count, plus the exact count as text

#### Scenario: Never-pressed bound entry still shows an indicator
- **WHEN** `trackShortcutUsage` is `true` and a bound entry has no recorded presses
- **THEN** the entry displays the lowest glyph level and the count `0`, in its normal position

#### Scenario: Indicator is absent when tracking is off
- **WHEN** `trackShortcutUsage` is `false`
- **THEN** no entry displays a usage glyph or count, and the grid renders exactly as it did before this change
